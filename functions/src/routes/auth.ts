...
authRouter.post("/device/start", async (req, res) => {
  try {
    ...
    return res.json(deviceRes.data);
  } catch (e: any) {
    console.error("Device flow start error:", e?.response?.data || e?.message);
    return res.status(400).json({error: "Device flow start failed", details: e?.response?.data || e?.message});
  }
});
...
authRouter.post("/device/poll", async (req, res) => {
  try {
    ...
    await db.collection("allegroAccounts").doc(accountData.id).set(accountData);

    return res.json({ok: true, account: accountData});
  } catch (e: any) {
    return res.status(400).json({error: "Device flow poll failed", details: e?.response?.data || e?.message});
  }
});
...
authRouter.post("/callback", async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({error: "Token exchange failed", details: e?.response?.data || e?.message});
  }
});
...
authRouter.post("/refresh", async (req, res) => {
  try {
    ...
    let accountDoc;
    if (accountId) {
      accountDoc = await db.collection("allegroAccounts").doc(accountId).get();
      if (!accountDoc.exists) {
        return res.status(404).json({error: "Account not found"});
      }
    } else {
      const accountsSnapshot = await db.collection("allegroAccounts").limit(1).get();
    ...
    return res.status(400).json({error: "Refresh failed", details: e?.response?.data || e?.message});
  }
});
...
authRouter.post("/client-credentials", async (req, res) => {
  try {
    ...
    return res.status(400).json({error: "Client credentials failed", details: e?.response?.data || e?.message});
  }
});

authRouter.get("/accounts", async (req, res) => {
  try {
    const accountsSnapshot = await db.collection("allegroAccounts").get();
