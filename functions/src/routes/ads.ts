...
import {getAccountToken as getToken} from "../utils/allegro-api";
...
// Helper to get account and access token (uses centralized token refresh)
async function getAccountToken(accountId: string) {
  ...
  // Get fresh token using centralized function
  const accessToken = await getToken(accountId);
...
adsRouter.get("/debug-schedules", async (req, res) => {
  try {
    ...
    schedules.forEach((s: any) => {
      console.log(
        `  ${s.name}: changeValue=${s.changeValue} ` +
        `(type: ${typeof s.changeValue}), changeMode=${s.changeMode}`
      );
    });
